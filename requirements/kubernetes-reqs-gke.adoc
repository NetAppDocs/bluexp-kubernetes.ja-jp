...
BlueXPコネクタ:: 必要な権限を持つ Connector が Google で実行されている必要があります。  a Connector,詳細は以下をご覧ください。
...
RBAC 許可:: BlueXPは、Active Directoryを使用するかどうかに関係なく、RBAC対応のクラスタをサポートしています。BlueXPコネクタの役割は、各GKEクラスタで許可されている必要があります。  up RBAC authorization,詳細は以下をご覧ください。
