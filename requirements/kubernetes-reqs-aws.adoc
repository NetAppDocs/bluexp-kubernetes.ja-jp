...
Cloud Manager Connector の略:: 必要な権限を持つコネクタが AWS で実行されている必要があります。  a Connector,詳細は以下をご覧ください。
...
RBAC 許可:: Cloud Manager Connector ロールは、各 Kubernetes クラスタで許可されている必要があります。  up RBAC authorization,詳細は以下をご覧ください。
