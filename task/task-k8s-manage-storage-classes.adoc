...
. ストレージクラスの名前を指定します。
. 「* Filesystem *」または「* Block * storage」を選択します。*Block* storageで、File System Type（fsstype）を選択します。
+
image:screenshot-k8s-storage-fstypes.png["Kubernetesクラスタストレージクラスを設定する際のブロックストレージのfsstypeオプションを示すスクリーンショット"]
